...
export default function AdminCommissionsTab() {
  ...
  const saveSettings = async () => {
    try {
      await updateSettings({ commission_percentage: parseFloat(editPercentage), min_payout: parseFloat(editMinPayout), commission_type: editType });
  ...
  const filteredCommissions = commissions.filter(c => !affiliateSearch || c.affiliate_id.toLowerCase().includes(affiliateSearch.toLowerCase()));
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card className="border-border"><CardContent className="p-4"><div className="flex items-center gap-2 mb-1"><TrendingUp className="h-4 w-4 text-primary" /><span className="text-xs text-muted-foreground">{t("adminCommissions.totalGenerated")}</span></div><div className="text-xl font-bold text-primary">{summary.total.toFixed(2)} €</div></CardContent></Card>
        <Card className="border-border"><CardContent className="p-4"><div className="flex items-center gap-2 mb-1"><Clock className="h-4 w-4 text-muted-foreground" /><span className="text-xs text-muted-foreground">{t("adminCommissions.pending")}</span></div><div className="text-xl font-bold text-muted-foreground">{summary.pending.toFixed(2)} €</div></CardContent></Card>
        <Card className="border-border"><CardContent className="p-4"><div className="flex items-center gap-2 mb-1"><CircleCheck className="h-4 w-4 text-primary" /><span className="text-xs text-muted-foreground">{t("adminCommissions.approved")}</span></div><div className="text-xl font-bold text-primary">{summary.approved.toFixed(2)} €</div></CardContent></Card>
        <Card className="border-border"><CardContent className="p-4"><div className="flex items-center gap-2 mb-1"><DollarSign className="h-4 w-4 text-primary" /><span className="text-xs text-muted-foreground">{t("adminCommissions.paid")}</span></div><div className="text-xl font-bold text-primary">{summary.paid.toFixed(2)} €</div></CardContent></Card>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <Input placeholder={t("adminCommissions.searchAffiliate")} value={affiliateSearch} onChange={(e) => setAffiliateSearch(e.target.value)} className="pl-9" />
        </div>
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-[150px]"><SelectValue placeholder={t("adminCommissions.status")} /></SelectTrigger>
          <SelectContent>
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-2 p-2 rounded-lg bg-muted/50 border border-border">
          <span className="text-sm text-muted-foreground">{t("adminCommissions.selected", { count: selected.size })}</span>
          <Button size="sm" variant="outline" onClick={() => handleBulkAction("approved")} className="text-xs">{t("adminCommissions.approve")}</Button>
          <Button size="sm" variant="outline" onClick={() => handleBulkAction("paid")} className="text-xs">{t("adminCommissions.markPaid")}</Button>
        </div>
      )}
      ...
      <Card className="border-border">
        <CardContent className="p-0">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="w-10"><Checkbox checked={selected.size === filteredCommissions.length && filteredCommissions.length > 0} onCheckedChange={toggleAll} /></TableHead>
            ...
            <TableBody>
              {loading ? (
                <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">{t("adminCommissions.loading")}</TableCell></TableRow>
              ) : filteredCommissions.length === 0 ? (
                <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">{t("adminCommissions.noCommissions")}</TableCell></TableRow>
              ) : (
                filteredCommissions.map((c) => (
                  <TableRow key={c.id}>
                    ...
                    <TableCell><Badge variant={STATUS_BADGE[c.status]?.variant || "secondary"} className="text-xs">{STATUS_BADGE[c.status]?.label || c.status}</Badge></TableCell>
      ...
      <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div><Label>{t("adminCommissions.commissionPercentage")}</Label><Input type="number" value={editPercentage} onChange={(e) => setEditPercentage(e.target.value)} min="0" max="100" step="0.5" /></div>
            <div><Label>{t("adminCommissions.minPayout")}</Label><Input type="number" value={editMinPayout} onChange={(e) => setEditMinPayout(e.target.value)} min="0" step="5" /></div>
            <div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setSettingsOpen(false)}>{t("adminCommissions.cancel")}</Button>
      ...
      <Dialog open={manualOpen} onOpenChange={setManualOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div><Label>{t("adminCommissions.userId")}</Label><Input value={manualUserId} onChange={(e) => setManualUserId(e.target.value)} placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" /></div>
            <div><Label>{t("adminCommissions.paymentAmountLabel")}</Label><Input type="number" value={manualAmount} onChange={(e) => setManualAmount(e.target.value)} placeholder="49.99" min="0" step="0.01" /></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setManualOpen(false)}>{t("adminCommissions.cancel")}</Button>
