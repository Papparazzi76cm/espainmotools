...
const HowItWorksSection = () => {
  ...
  const steps = [
    { number: "01", title: t("howItWorks.step1Title"), description: t("howItWorks.step1Desc"), image: step1Img },
    { number: "02", title: t("howItWorks.step2Title"), description: t("howItWorks.step2Desc"), image: step2Img },
    { number: "03", title: t("howItWorks.step3Title"), description: t("howItWorks.step3Desc"), image: step3Img },
  ];
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-20">
          <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-primary/30 bg-primary/10 text-primary text-xs font-medium mb-4">
            <Rocket className="h-3 w-3" /> {t("howItWorks.badge")}
          </span>
        ...
        <div className="relative">
          ...
          <div className="space-y-20 lg:space-y-28">
            {steps.map((step, index) => {
              ...
              return (
                <motion.div key={step.number} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6, delay: 0.1 }} className={`flex flex-col lg:flex-row items-center gap-8 lg:gap-16 ${!isEven ? "lg:flex-row-reverse" : ""}`}>
                  <div className={`flex-1 ${isEven ? "lg:text-right" : "lg:text-left"}`}>
                    <motion.div initial={{ opacity: 0, x: isEven ? -30 : 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.2 }}>
                      <span className="text-5xl sm:text-6xl font-black text-primary/20 block mb-2" style={{ animation: "text-glow-pulse 3s ease-in-out infinite", animationDelay: `${index * 0.8}s` }}>{step.number}</span>
                      <h3 className="text-xl sm:text-2xl font-bold text-foreground mb-3">{step.title}</h3>
                      <p className="text-muted-foreground leading-relaxed max-w-md mx-auto lg:mx-0">{step.description}</p>
                    </motion.div>
                  </div>
                  ...
                  <div className="flex-1">
                    <motion.div initial={{ opacity: 0, x: isEven ? 30 : -30, scale: 0.95 }} whileInView={{ opacity: 1, x: 0, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.3 }} whileHover={{ scale: 1.03, y: -4 }} className="relative group">
                      ...
                      <div className="relative rounded-2xl overflow-hidden border border-border/40 bg-card/50 shadow-2xl shadow-primary/5 group-hover:shadow-primary/20 transition-shadow duration-500">
                        <img src={step.image} alt={step.title} width={960} height={640} loading="lazy" className="w-full h-auto" />
