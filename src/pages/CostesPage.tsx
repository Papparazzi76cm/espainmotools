...
const CostesPage = () => {
  ...
  const calcular = () => {
    ...
    const res = {
      comprador: { impuestos: impuestosComprador, impuestoLabel, notaria, registro, gestoria, total: impuestosComprador + notaria + registro + gestoria },
      vendedor: { plusvalia, irpf, comision: comisionMonto, total: plusvalia + irpf + comisionMonto },
    };
    setResultado(res);
    saveResult(`${fmt(p)} — ${ccaa?.label || "General"}`, { precio: p, operacion, comunidad, esObraNueva, comision }, res);
  };
    ...
    <div className="max-w-4xl mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <Select value={operacion} onValueChange={setOperacion}>
                  ...
                  <SelectContent><SelectItem value="compraventa">Compraventa</SelectItem><SelectItem value="alquiler">Alquiler</SelectItem></SelectContent>
                </Select>
              </div>
              <div>
              ...
              <Button onClick={calcular} className="w-full">Calcular Costes</Button>
            </CardContent>
          </Card>
        ...
        {resultado ? (
          <>
            <Card className="glass-card">
              <CardHeader className="pb-2"><div className="flex items-center gap-2"><ArrowDown className="h-4 w-4 text-primary" /><CardTitle className="text-base">Comprador</CardTitle></div></CardHeader>
              <CardContent className="space-y-3">
                ...
                <Separator />
                <div className="flex justify-between font-semibold"><span>Total</span><div className="text-right"><div className="text-primary">{fmt(resultado.comprador.total)}</div></div></div>
              </CardContent>
            </Card>
            <Card className="glass-card">
              <CardHeader className="pb-2"><div className="flex items-center gap-2"><ArrowUp className="h-4 w-4 text-primary" /><CardTitle className="text-base">Vendedor</CardTitle></div></CardHeader>
              <CardContent className="space-y-3">
                ...
                <Separator />
                <div className="flex justify-between font-semibold"><span>Total</span><div className="text-right"><div className="text-primary">{fmt(resultado.vendedor.total)}</div></div></div>
              </CardContent>
            </Card>
          </>
        ) : (
          <Card className="glass-card lg:col-span-2">
            <CardContent className="p-8 text-center text-muted-foreground">
              ...
              <div className="mt-4 p-3 rounded-lg bg-muted/50 border border-border text-left">
                <div className="flex items-start gap-2"><Info className="h-4 w-4 text-primary mt-0.5 flex-shrink-0" /><p className="text-xs text-muted-foreground">Los cálculos son estimaciones basadas en la fiscalidad española vigente.</p></div>
              </div>
            </CardContent>
          </Card>
        )}
...
function CostLine({ label, value, fmt }: { label: string; value: number; fmt: (n: number) => string }) {
  return (<div className="flex justify-between text-sm"><span className="text-muted-foreground">{label}</span><div className="text-right"><div>{fmt(value)}</div></div></div>);
}
